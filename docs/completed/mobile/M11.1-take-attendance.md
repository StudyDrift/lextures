...
| **Status (today)**   | DONE — staff take roll on iOS and Android with roster marking, summary, offline save, and section scoping |
| **Estimated effort** | S                                                                                                            |
...
- **API** — Added `createAttendanceSession`, `saveAttendanceRecords`, `closeAttendanceSession`, and `fetchCourseSections` to `LMSAPIFeatures.swift` / `LmsApi.kt`.
- **Logic** — `TakeAttendanceLogic` (summary counts, mark-all-present, today's session lookup, save payload).
- **iOS** — `TakeAttendanceView` with segmented status chips, staff entry from `CourseAttendanceSection`, roll-call sessions route to take mode.
- **Android** — `TakeAttendanceScreen` parity; staff CTA in `CourseAttendanceSection`; saves/close via `OfflineService.enqueueMutation`.
- **Tests** — `TakeAttendanceLogicTests` / `TakeAttendanceLogicTest`. Android `:app:testDebugUnitTest` passes.
- **i18n** — `mobile.attendance.take.*` keys in `clients/mobile/locales/*.json` (synced to iOS/Android).
