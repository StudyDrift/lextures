...
> CLI parity plan. Source: `admin_audit_log_http.go` (`admin/audit-log`, `compliance/audit-log`), `registerImpersonationRoutes` (`admin-console/impersonate`), `admin_search.go` (`admin/search`), `impersonationWriteBlockMiddleware`. Baseline: `clients/cli/cmd/audit_log.go`, `audit_impersonation_logic.go`, `impersonate.go`, `admin.go`, `internal/auth/impersonation.go`, `audit_impersonation_test.go`.
