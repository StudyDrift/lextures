...
// Publisher enqueues SMS notification jobs.
type Publisher interface {
	Publish(ctx context.Context, msg QueueMessage) error
	Close() error
}

// Consumer receives SMS notification jobs.
...
func (b *Bus) Publish(ctx context.Context, msg QueueMessage) error {
	if b == nil || b.pub == nil {
		return fmt.Errorf("sms notification queue not configured")
	}
	...
	for {
		...
		case msg := <-m.ch:
			...
			go func(msg QueueMessage) {
				...
				if err := handler(msg); err != nil {
					slog.Warn("sms_notification_queue: memory handler failed", "user_id", msg.UserID, "event_type", msg.EventType, "err", err)
				}
			}(msg)
		}
	}
