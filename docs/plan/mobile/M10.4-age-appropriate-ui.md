...
- **FR-2.** In this mode, core young-student flows MUST use **larger targets, simplified
  navigation (fewer drawer destinations/options), icon-forward affordances, and simpler copy**.
...
**iOS** — `Core/Design/UIMode.swift` + theme/type variants *(new)*, simplified drawer in `Features/Navigation/GlobalDrawer.swift` (fewer `DrawerGroup`s from `Core/Routing/MobileDestinations.swift`) + `MainTabView.swift`, mode-aware copy in core young flows, settings toggle.

**Android** — `core/design/UIMode.kt` + theme variants *(new)*, simplified drawer in `features/navigation/GlobalDrawer.kt` (fewer groups from `core/navigation/MobileDestinations.kt`) + `features/home/HomeScreen.kt`, mode-aware young flows, settings toggle.
