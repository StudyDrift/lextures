...
> CLI parity plan. Source: `registerTutorRoutes` + `registerPersistentTutorRoutes` (`courses/{id}/tutor`), `registerStudyBuddyRoutes` (`courses/{id}/study-buddy`, `me/study-*`), `registerDiagnosticRoutes` (`diagnostic-attempts`, `courses/{id}/diagnostic-config`), `registerConceptRoutes` (`concepts`), `registerLearningPathRoutes` (`paths`, `me/paths`), `registerLearnerRoutes` (`learners`). Baseline: `clients/cli/cmd/tutor_study_buddy.go`, `tutor_study_buddy_logic.go`, `tutor_study_buddy_test.go`.
