...
mode. Role detection in the app shell selects the parent drawer destinations. Calls in
`LMSAPIFeatures.swift`/`LmsApi.kt`.

## 9. Files to touch

**iOS** — `Features/Parent/ParentDashboardView.swift` *(new)*, `Features/Home/MainTabView.swift` + `Features/Navigation/GlobalDrawer.swift` (role-based drawer shell; parent `DrawerGroup`s in `Core/Routing/MobileDestinations.swift`), reuse grades/attendance, `Core/LMS/LMSAPIFeatures.swift`, `project.pbxproj`.

**Android** — `features/parent/ParentDashboardScreen.kt` *(new)*, `features/home/HomeScreen.kt` + `features/navigation/GlobalDrawer.kt` (role-based drawer shell; parent groups in `core/navigation/MobileDestinations.kt`), reuse grades/attendance, `core/lms/LmsApi.kt`.
