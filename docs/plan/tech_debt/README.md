...
**No user-visible behaviour may change.** Every story in this folder is a *refactor* or a *deletion of proven-dead code*, with exactly one exception ([TD.3](TD.3-repair-and-verify-openapi-contract.md), which repairs a confirmed shipped defect). A story is not "done" because the code looks nicer — it is done when the guardrails in [TD.1](TD.1-refactoring-safety-net.md) prove the observable contract is byte-identical.
...
| `/api/openapi.json` serves **invalid JSON** | confirmed at `HEAD` — see [TD.3](TD.3-repair-and-verify-openapi-contract.md) |
| API documented vs implemented | **226** documented paths vs **1,407** registered routes (**16%**) |
...
|---|---|---|---|
| TD.1 | [Refactoring safety net](TD.1-refactoring-safety-net.md) | M | Route-inventory + response-shape snapshots. Nothing else in this folder is safe without it. |
| TD.2 | [Convention charter & automated enforcement](TD.2-convention-charter-and-enforcement.md) | S | Codifies the target state and adds ratchets so fixed debt stays fixed. |

### Phase 1 — Confirmed defects & proven-dead code (low risk, immediate value)

| ID | Plan | Effort | Depends on |
|---|---|---|---|
| TD.3 | [Repair and verify the OpenAPI contract](TD.3-repair-and-verify-openapi-contract.md) | S | TD.1 |
...
4. **The ratchet only tightens.** New budgets in [TD.2](TD.2-convention-charter-and-enforcement.md) are enforced against *new and modified* code first; the backlog burns down per story. Never loosen a threshold to make CI pass.
