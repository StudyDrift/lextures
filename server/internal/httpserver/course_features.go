...
func (d Deps) handlePatchCourseFeatures() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		filesEnabled := existing.FilesEnabled
		if req.FilesEnabled != nil {
			filesEnabled = *req.FilesEnabled
		}
		attendanceEnabled := existing.AttendanceEnabled
		if req.AttendanceEnabled != nil {
			attendanceEnabled = *req.AttendanceEnabled
		}
		whiteboardEnabled := existing.WhiteboardEnabled
		...
		screenShareEnabled := existing.ScreenShareEnabled
