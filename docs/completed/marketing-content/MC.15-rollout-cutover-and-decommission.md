...
| **Severity** | MAJOR |
| **Markets** | K12 / HE / HS |
| **Status (today)** | COMPLETED — API-only build path, decommission, CI guardrails, archive provenance, runbook, docs, and coverage manifest shipped |
...
> Completed repository implementation. Production-only evidence (named RBAC grants, snapshots,
> Search Console watch, Rich Results samples, and release approvals) is retained in the restricted
> release record described by `docs/runbooks/marketing-content-cutover.md`; secrets and personal
> data are intentionally not committed.

> Deployment addendum: the final corpus import is packaged as idempotent migration
> `485_marketing_content_seed.sql`, generated from the archived commit. It runs through the normal
> server migration path so operators without a production database shell still receive all 70
> articles. The eight screenshots referenced by the imported Markdown remain static until they are
> replaced through the media workspace.
...
- Related plans: every plan in this folder; in particular
  [MC.6](../../completed/marketing-content/MC.6-markdown-to-database-migration.md) (parity harness),
  [MC.7](MC.7-www-build-time-content-integration.md) (`WWW_CONTENT_SOURCE`),
  [MC.8](../../completed/marketing-content/MC.8-publish-pipeline-and-scheduling.md) (dispatch configuration),
  [MC.12](../../completed/marketing-content/MC.12-seo-parity-from-database.md) (verification surface).
