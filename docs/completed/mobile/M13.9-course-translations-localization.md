...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — locale variants, glossary add/edit, and coverage review on iOS and Android; gated by `translationMemoryEnabled` |
...
- **Logic**: `CourseTranslationsLogic` (iOS/Android) — feature gate, locale merge/track, glossary validate/diff/filter/paginate, coverage percent, RTL locale helpers, offline cache + idempotency keys.
- **API**: `LMSAPICourseTranslations.swift` + `LmsApi` methods for `GET /translation-coverage`, `GET /translations`, `GET|POST /glossary`.
- **Models**: `LMSFeatureModelsCourseTranslations` / `LmsFeatureModelsCourseTranslations.kt`.
- **UI**: `CourseTranslationsSettingsView` / `CourseTranslationsSettingsScreen` wired into M13.1 settings shell (`translations` section when `translationMemoryEnabled`). Locales list with coverage % → locale detail (coverage ring/bar, read-only item status list, searchable glossary with add/edit sheet). Offline queued glossary upserts.
- **Scope**: Locale + glossary management + coverage review. Full segment authoring remains web-only (plan non-goal). Glossary delete is not exposed by the API; add/edit uses POST upsert.
- **Tests**: `CourseTranslationsLogicTests` (iOS), `CourseTranslationsLogicTest` (Android). Unit tests pass; Android `compileDebugKotlin` and mobile locale sync clean.
