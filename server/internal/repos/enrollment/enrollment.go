...
// GetActiveEnrollmentID returns any active enrollment id for the user in the course
// (staff or student). Used for Content Tools preview-as-student state (CT.2).
func GetActiveEnrollmentID(ctx context.Context, pool *pgxpool.Pool, courseID, userID uuid.UUID) (*uuid.UUID, error) {
	var id uuid.UUID
	err := pool.QueryRow(ctx, `
SELECT ce.id
FROM course.course_enrollments ce
