...
> Implementation plan. Digitally signed, QR-verifiable documents + a third-party verifier portal. Builds on the shipped CLR/VC signing. Source landscape: [transcripts/README](../../plan/transcripts/README.md).
...
| **Severity** | MAJOR |
| **Markets** | HE · K12 · SL |
| **Status (today)** | DONE — official transcripts signed as W3C VCs (`SignTranscriptCredential`); PDF QR + content-hash footer; unified `/verify` portal (link/code/upload); revoke/unrevoke; `verifications` audit log; CLR path unified; migration `410`. |
...
Migration `410_transcript_verifications.sql`:
...
ALTER TABLE transcripts.transcript_documents
    ...
    ADD COLUMN disclose_publicly BOOLEAN NOT NULL DEFAULT FALSE;
```
...
- Implementation: `server/migrations/410_transcript_verifications.sql`, `server/internal/service/transcriptverify/`, `server/internal/service/vc_signing/transcript.go`, `server/internal/httpserver/transcripts_verify_http.go`, `clients/web/src/pages/verify/CcrVerify.tsx`.
