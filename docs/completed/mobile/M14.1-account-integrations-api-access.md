...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — Integrations settings on iOS and Android (access keys, calendar feeds, MCP, admin service tokens) |
...
- **Logic & API**: `AccountIntegrationsLogic` + `LMSFeatureModelsAccountIntegrations` / `LmsFeatureModelsAccountIntegrations.kt`; `LMSAPIAccountIntegrations.swift` / extended `LmsApi.kt` for access keys, MCP config, calendar token (reuse), and admin service tokens.
- **UI**: `IntegrationsView` / `IntegrationsScreen` with four sections (access keys, calendar subscriptions, MCP, service tokens). One-time secret reveal modal with secure clipboard auto-clear (60s).
- **Profile entry**: `IntegrationsEntryCard` (iOS) and Profile tab nav row (Android), gated by `ffMobileSettingsIntegrations`.
- **Sub-gating**: `ffApiTokens` (access keys + MCP), `ffCalendarFeeds` (calendar section), `global:app:rbac:manage` + API 403 probe (service tokens).
- **Feature flag**: `ffMobileSettingsIntegrations` on `PlatformFeatures` / `MobilePlatformFeatures` (default off).
- **Tests**: `AccountIntegrationsLogicTests` (iOS), `AccountIntegrationsLogicTest` (Android). iOS + Android builds pass; unit tests pass.
