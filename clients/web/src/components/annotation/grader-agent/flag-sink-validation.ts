...
/** Whether a source may wire into a Flag for Review input slot. */
export function flagSinkSourceIsValid(
  sourceType: string,
  sourceHandle: string,
  targetHandle: string,
): boolean {
...
export function graphHasFlagSink(nodes: { type: string }[]): boolean {
  return nodes.some((node) => isFlagForReviewNodeType(node.type))
}