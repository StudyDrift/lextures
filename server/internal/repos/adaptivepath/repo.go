...
SELECT r.id, r.structure_item_id, r.rule_type::text,
       r.concept_ids,
       (r.threshold)::float8,
...
func InsertRule(
	...
	targetItemID *uuid.UUID,
	priority int16,
) (*StructurePathRuleRow, error) {
	var r StructurePathRuleRow
	err := pool.QueryRow(ctx, `
INSERT INTO course.structure_item_path_rules
  (structure_item_id, rule_type, concept_ids, threshold, target_item_id, priority)
VALUES ($1, $2::course.path_rule_type, $3, $4, $5, $6)
RETURNING id, structure_item_id, rule_type::text,
          concept_ids, (threshold)::float8, target_item_id, priority, created_at
`, structureItemID, ruleType, conceptIDs, threshold, targetItemID, priority).Scan(
		&r.ID, &r.StructureItemID, &r.RuleType,
		&r.ConceptIDs, &r.Threshold, &r.TargetItemID, &r.Priority, &r.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
	if r.ConceptIDs == nil {
		r.ConceptIDs = []uuid.UUID{}
	}
	return &r, nil
}
...
func DeletePathOverride(ctx context.Context, pool *pgxpool.Pool, enrollmentID uuid.UUID) (bool, error) {
	tag, err := pool.Exec(ctx, `
DELETE FROM course.enrollment_path_overrides WHERE enrollment_id = $1
`, enrollmentID)
	if err != nil {
		return false, err
	}
...
func InsertPathEvent(
	...
	_, err := pool.Exec(ctx, `
INSERT INTO course.learner_path_events
  (enrollment_id, from_item_id, to_item_id, rule_id, was_override, was_fallback)
VALUES ($1, $2, $3, $4, $5, $6)
...
func scanRuleRows(rows ruleScanner) ([]StructurePathRuleRow, error) {
	out := make([]StructurePathRuleRow, 0)
	for rows.Next() {
		var r StructurePathRuleRow
		if err := rows.Scan(
			&r.ID, &r.StructureItemID, &r.RuleType,
