...
| **Status (today)** | **DONE** — My Paths, path runner, catalog browse/enroll, dashboard recommendations on iOS and Android |
| **Estimated effort** | M |
...
| **Depends on** | [M3.1](../completed/mobile/M3.1-module-content-viewer.md) (path steps are content/items), [M9.1](../plan/mobile/M9.1-catalog-enroll.md) (paid path checkout handoff) |
...
- **API layer**: `LMSAPIPaths.swift` / `LmsApi.kt` — catalog paths, my paths, path progress, enroll, recommendation events. Models in `LMSFeatureModelsPaths.swift` / `LmsFeatureModelsPaths.kt`. `PathsLogic` for next-step/lock state and recommendation merge.
- **Screens**: `MyPathsView`, `PathRunnerView`, `PathsCatalogView`, `PathLandingView`, `DashboardStudySection` (iOS); `MyPathsScreen`, `PathRunnerScreen`, `PathsCatalogScreen`, `PathLandingScreen`, `DashboardStudySection` (Android).
- **Navigation**: More → Learning paths wired when `ffLearningPaths` is enabled. Dashboard shows enrolled-path progress card + “what’s next” recommendations (continue/review/strengthen/challenge surfaces).
- **Enroll**: Free paths enroll natively; paid paths show web checkout handoff (`AppConfiguration.webURL`).
- **Offline**: `OfflineCacheKey.myPaths()`, `pathProgress()`, `catalogPaths()` cache read paths.
- **Tests**: `PathsLogicTests` (iOS), `PathsLogicTest` (Android).
- **i18n**: `mobile.paths.*` in all five `clients/mobile/locales/*.json` (synced).
</content>
