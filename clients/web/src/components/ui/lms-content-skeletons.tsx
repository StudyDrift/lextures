        ...
        <div className="mt-4 rounded-2xl border border-border-default bg-surface-raised p-5 shadow-sm dark:border-border-default dark:bg-surface-raised">
          ...
          <ul className="mt-4 space-y-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <li key={i} className="rounded-xl border border-border-subtle px-3 py-3 dark:border-border-subtle">
  ...
  return (
    <div className="mt-8 space-y-10" aria-busy="true">
      ...
      <div>
        ...
        <div className="mt-3 rounded-2xl border border-border-default bg-surface-raised p-5 shadow-sm dark:border-border-default dark:bg-surface-raised">
        ...
        <div className="mt-4 rounded-2xl border border-border-default bg-surface-raised p-5 shadow-sm dark:border-border-default dark:bg-surface-raised">
          ...
          <ul className="mt-4 space-y-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <li
                key={i}
                className="flex flex-col gap-2 rounded-xl border border-border-subtle px-3 py-3 dark:border-border-subtle sm:flex-row sm:items-center sm:justify-between"
              >
    ...
    <div className="mt-8" aria-busy="true">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="flex h-full flex-col overflow-hidden rounded-2xl border border-border-default bg-surface-raised shadow-sm dark:border-border-default dark:bg-surface-raised"
          >
...
export function GradebookLoadingSkeleton() {
  ...
  return (
    <div className="mt-6 space-y-3" aria-busy="true">
      ...
      <div className="overflow-hidden rounded-xl border border-border-default bg-surface-raised shadow-sm dark:border-border-default dark:bg-surface-raised">
        <div className="min-w-max">
          <div className="flex border-b border-border-default bg-surface-base dark:border-border-default dark:bg-surface-overlay">
            <div className="sticky start-0 z-10 flex w-48 shrink-0 items-end border-e border-border-default bg-surface-base px-3 py-3 dark:border-border-default dark:bg-surface-overlay">
              <Shimmer className="h-4 w-20" />
            </div>
            <div className="flex w-20 shrink-0 items-end border-e border-border-default bg-surface-base px-2 py-3 dark:border-border-default dark:bg-surface-overlay">
              <Shimmer className="h-3 w-10" />
            </div>
            {colPlaceholders.map((_, i) => (
              <div
                key={i}
                className="flex w-36 shrink-0 items-end border-e border-border-default bg-surface-base px-2 py-3 last:border-e-0 dark:border-border-default dark:bg-surface-overlay"
              >
          ...
          {rowPlaceholders.map((_, ri) => (
            <div
              key={ri}
              className="flex border-b border-border-subtle last:border-b-0 dark:border-border-subtle"
            >
              <div className="sticky start-0 z-10 w-48 shrink-0 border-e border-border-default bg-surface-raised px-3 py-2.5 dark:border-border-default dark:bg-surface-raised">
                <Shimmer className="h-4 w-32" />
              </div>
              <div className="flex w-20 shrink-0 items-center border-e border-border-default px-2 py-2 dark:border-border-default">
                <Shimmer className="mx-auto h-4 w-10" />
              </div>
              {colPlaceholders.map((_, ci) => (
                <div
                  key={ci}
                  className="flex w-36 shrink-0 items-center border-e border-border-default px-2 py-2 last:border-e-0 dark:border-border-default"
                >
