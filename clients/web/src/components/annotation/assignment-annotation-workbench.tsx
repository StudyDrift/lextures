...
function AssignmentAnnotationWorkbenchInner({
  ...
  const documentPreviewContent =
    displayFilePath ? (
      ...
      <div className="flex h-full min-h-48 items-center justify-center rounded-lg border border-dashed border-border-default bg-surface-base px-4 py-6 text-sm text-fg-muted dark:border-border-default/60 dark:text-fg-muted">
        {mode === 'staff' ? (
          current?.submittedAt ? (
            <p>
      ...
      >
        ...
        >
          ...
          <ResizableSplitPane
            ...
            primary={
              <div className="h-full min-h-[40vh] overflow-auto bg-surface-base p-3/60">
                {annotationDocArea}
              </div>
            }
  ...
  return (
    ...
    >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {mode === 'staff' ? (
                <button
                  ...
                  className="rounded-md border border-border-default bg-surface-raised px-2.5 py-1 text-xs font-semibold text-accent-fg hover:bg-surface-base disabled:opacity-50 dark:border-border-default dark:bg-surface-base dark:text-indigo-300 dark:hover:bg-surface-raised"
                >
                  View report
                </button>
              ) : null}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {resubmissionWorkflowEnabled && current && current.submittedBy && (
              <button
                ...
                className="rounded-lg border border-border-strong bg-surface-raised px-3 py-1.5 text-xs font-semibold text-fg-default hover:bg-surface-base disabled:opacity-50 dark:border-border-default dark:bg-surface-raised dark:text-fg-default dark:hover:bg-neutral-800/80"
              >
                Request revision
              </button>
            )}
      ...
      {revisionFormOpen && resubmissionWorkflowEnabled && mode === 'staff' && current?.id && (
        <div className="rounded-lg border border-border-default bg-slate-50/90 p-4 dark:border-border-default/60">
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end">
            <label className="text-sm text-fg-default">
              <span className="mb-1 block text-xs font-semibold text-fg-muted">
                Resubmit by
              </span>
              <input
                type="datetime-local"
                className="rounded-md border border-border-strong bg-surface-raised px-2 py-1 text-sm dark:border-border-default dark:bg-surface-base"
                value={revDueLocal}
            ...
            <div className="min-w-0 flex-1">
              ...
              <textarea
                className="min-h-20 w-full rounded-md border border-border-strong bg-surface-raised px-2 py-1.5 text-sm dark:border-border-default dark:bg-surface-base"
                value={revFeedback}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              type="button"
              className="rounded-md bg-accent-solid px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50"
              disabled={revisionBusy}
            ...
            <button
              type="button"
              className="rounded-md border border-border-strong bg-surface-raised px-3 py-1.5 text-sm dark:border-border-default dark:bg-surface-raised"
              onClick={() => {
                setRevisionFormOpen(false)
                setRevDueLocal('')
                setRevFeedback('')
      ...
      {resubmissionWorkflowEnabled && mode === 'staff' && submissionVersions.length > 0 && current?.id ? (
        <div
          className="flex flex-wrap gap-1 border-b border-border-default pb-1 dark:border-border-default"
          role="tablist"
          ...
          {submissionVersions.map((v) => {
            ...
            return (
              <button
                ...
                className={`rounded-t-md px-2.5 py-1.5 text-xs font-semibold ${ active ? 'bg-surface-sunken text-fg-default dark:bg-surface-overlay' : 'text-fg-muted hover:bg-surface-base dark:text-fg-muted dark:hover:bg-neutral-800/60' }`}
      ...
      {isListedGrader && current ? (
        <div className="rounded-lg border border-border-default bg-slate-50/90 px-3 py-2 dark:border-border-default/60">
          <p className="text-xs font-semibold uppercase tracking-wide text-fg-muted">
            Provisional score
          </p>
          <div className="mt-2 flex flex-wrap items-end gap-2">
            <label className="text-sm text-fg-default" htmlFor="prov-score">
            ...
            <input
              ...
              className="w-28 rounded-md border border-border-strong bg-surface-raised px-2 py-1 text-sm dark:border-border-default dark:bg-surface-base"
            />
            <button
              type="button"
      ...
      {both ? (
        ...
        >
          ...
          <button
            ...
            className={`rounded-t-md px-3 py-1.5 text-sm font-medium ${ panel === 'media' ? 'bg-surface-sunken text-fg-default dark:bg-surface-overlay' : 'text-fg-muted hover:bg-surface-base dark:text-fg-muted dark:hover:bg-neutral-800/60' }`}
      ...
      {showDocPanel && mode === 'student' && submissionAllowsFile ? (
        <div className="flex flex-wrap items-center gap-3">
          <label
            className={`text-sm font-medium ${mine?.resubmissionRequested || !mine?.attachmentFileId ? 'text-fg-default' : 'text-fg-subtle'}`}
          >
            <span className="me-2">
              {mine?.resubmissionRequested
                ? 'Resubmit file'
        ...
        staffGradingSidebarActive && gradingSidebar ? (
          ...
          <>
            ...
            {studentFeedbackSidebar ? (
              <div className="min-h-[min(40vh,480px)] overflow-hidden rounded-xl border border-border-default">
                {studentFeedbackSidebar}
              </div>
            ) : null}
          </>
