...
| **Severity** | MAJOR |
| **Markets** | HE |
| **Status (today)** | MISSING — Lextures is send-only. There is no inbox for transcripts arriving from other schools, no PESC parsing, and no way to attach an incoming transcript to a prospective/transfer student for credit evaluation. |
...
Migration `384_transcript_inbound.sql` (indicative):
...
- Related plans: [T06](T06-electronic-delivery-standards.md), [T02](../../completed/transcripts/T02-recipient-directory-and-orders.md), [T10](T10-order-tracking-notifications.md).
