...
> Implementation plan. One learner-owned home for transcripts, diplomas, certificates, badges, and CLRs — portable after graduation. Source landscape: [transcripts/README](../../plan/transcripts/README.md).
...
| **Severity** | MAJOR |
| **Markets** | SL · HE · K12 |
| **Status (today)** | DONE — unified learner wallet index over transcripts/CLR/badges/certificates/CE; curated collections with disclosure-controlled share links + access history; async ZIP export; public `/wallet/s/{token}`; migration `411`. |
| **Estimated effort** | M (2–4w) |
| **Owner (proposed)** | Learner-experience squad |
| **Depends on** | T01 (transcripts), T08 (verification), [14.13 CLR](../14-higher-ed-specific/14.13-co-curricular-transcript.md), [B1 badges](../) |
...
Migration `411_credential_wallet.sql`. The wallet is primarily an **index/view** over
...
1. ~~Live UNION view vs. materialized `wallet_items` cache~~ — **resolved:** refresh-on-read into `credentials.wallet_items` (upsert + delete missing) so collections can reference stable IDs while status stays source-true.
2. ~~Alumni access model~~ — **resolved for launch:** any authenticated learner account can open `/me/wallet` (no enrollment gate); retention/deletion still follow platform DSAR/S02 policy.
3. External wallet interop (import/export to third-party VC wallets) at launch or later? — later; export ZIP already includes Open Badges / CLR / VC JSON where available.
...
- Related plans: [T01](T01-official-transcript-generation.md), [T08](T08-credential-verification.md), [T11](T11-diploma-certificate-issuance.md), [14.13](../14-higher-ed-specific/14.13-co-curricular-transcript.md).
- Implementation: `server/migrations/411_credential_wallet.sql`, `server/internal/repos/wallet/`, `server/internal/service/credentialwallet/`, `server/internal/httpserver/wallet_http.go`, `server/internal/background/jobqueue_wallet_export.go`, `clients/web/src/pages/lms/credential-wallet-page.tsx`, `clients/web/src/pages/public/wallet-share-page.tsx`.
