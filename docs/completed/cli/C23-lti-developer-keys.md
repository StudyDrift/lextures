...
> CLI parity plan. Source: `registerLTIHTTPRoutes` (`lti`, 10), `saml_lti.go`, `courses/{id}/lti-external-tools`, `developer` (2 — developer keys), `admin/lti` (7). Baseline: `clients/cli/cmd/lti.go`, `dev_keys.go`, `lti_developer_keys_logic.go`, `lti_developer_keys_test.go`.
