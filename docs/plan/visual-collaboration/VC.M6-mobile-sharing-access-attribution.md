...
> Implementation plan. Source: mobile parity for board access/sharing. Landscape: [visual-collaboration/README](README.md). Mirrors web [VC.6](../../completed/visual-collaboration/VC.6-sharing-access-contributors.md); consumes the single server-side **access resolver** and the public board-link endpoints, and adds QR/deep-link handling for phones.
...
- **Reuse**: native share sheet + universal/app-link routing already configured for other deep links
  ([M0.1](../../completed/mobile/M0.1-push-deep-links.md)); camera for QR scan (if in scope); the server access
...
- **New (Android)**: `core/lms/BoardAccessApi.kt`, `features/boards/BoardShareSheet.kt`,
  `features/boards/public/BoardPublicScreen.kt`; register the board-link URL scheme in the manifest.
...
- Web plan: [VC.6](../../completed/visual-collaboration/VC.6-sharing-access-contributors.md); server resolver
  `server/internal/repos/board/access.go`, public `board_links_http.go`; web `share-dialog.tsx`,
  `pages/public/board-share-page.tsx`.
- Related mobile plans: [VC.M2](VC.M2-mobile-posts-and-content.md), [VC.M5](VC.M5-mobile-reactions-comments-assessment.md), [VC.M7](VC.M7-mobile-moderation-safety.md); deep links [M0.1](../../completed/mobile/M0.1-push-deep-links.md).
