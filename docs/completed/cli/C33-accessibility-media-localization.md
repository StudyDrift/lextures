...
> CLI parity plan. Source: `accessibility_http.go` (`accessibility`), `alt_text_http.go` (`alt-text`), `registerCaptionRoutes`/`registerCaptionAccessibilityRoutes`, `registerTTSRoutes` (`tts`), `stt`, `registerReadingLevelRoutes`, `registerTranscodeRoutes`, `registerTusRoutes`, `registerTranslationRoutes` (`translate`, `translation-memory`), `course_translation.go` (`courses/{id}/translations`, `translation-coverage`), `settings/locale`, `timezones`. Baseline: `clients/cli/cmd/accessibility_media_localization.go`, `accessibility_media_localization_logic.go`, `accessibility_media_localization_test.go`.
