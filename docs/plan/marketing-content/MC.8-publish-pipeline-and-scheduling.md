...
| **Severity** | MAJOR |
| **Markets** | K12 / HE / HS |
| **Status (today)** | MISSING — publishing means merging a PR; the deploy workflow is triggered by a `www/**` path change only |
...
- Related plans: [MC.2](MC.2-authoring-api-and-revisions.md),
  [MC.7](MC.7-www-build-time-content-integration.md), [MC.9](MC.9-marketing-content-workspace-shell.md),
