  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/30" role="dialog" aria-modal>
      <div className="flex h-full w-full max-w-md flex-col bg-surface-raised shadow-xl dark:bg-surface-raised">
        ...
        <div className="border-b border-border-default p-4 dark:border-border-default">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <p className="text-sm text-fg-muted">{t('liveQuiz.editor.bankEmpty')}</p>
          ) : (
            <ul className="space-y-2">
              {rows.map((row) => (
                <li key={row.id}>
                  <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-md border border-border-default p-3 dark:border-border-default">
                    <input
                      type="checkbox"
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="text-xs text-fg-muted">{row.questionType}</span>
                    </span>
                  </label>
                </li>
              ))}
        ...
        <div className="border-t border-border-default p-4 dark:border-border-default">
          <button
            ...
            className="w-full min-h-11 rounded-md bg-accent-solid px-3 py-2 text-sm font-medium text-white disabled:opacity-50"
          >
