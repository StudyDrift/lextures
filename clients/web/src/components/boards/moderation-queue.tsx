...
export function BoardModerationQueue({ open, onClose, courseCode, boardId, onChanged }: Props) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-lg border border-border-default bg-surface-raised shadow-xl dark:border-border-default dark:bg-surface-raised">
        <div className="flex items-center justify-between border-b border-border-default px-4 py-3 dark:border-border-default">
          <h2 id="board-moderation-title" className="text-lg font-semibold text-fg-default">
        ...
        <div className="flex gap-1 border-b border-border-default px-2 pt-2 dark:border-border-default" role="tablist">
          {tabs.map((item) => (
            <button
              key={item.id}
              type="button"
              ...
              className={`rounded-t-md px-3 py-2 text-sm font-medium ${ tab === item.id ? 'bg-surface-sunken text-fg-default dark:bg-surface-overlay dark:text-fg-default' : 'text-fg-muted hover:bg-surface-base dark:text-fg-muted dark:hover:bg-neutral-800/60' }`}
            >
...
function PendingList({
  ...
  const { t } = useTranslation('common')
  if (posts.length === 0) {
    return <p className="text-sm text-fg-muted">{t('boards.moderation.emptyPending')}</p>
  }
  return (
    <ul className="space-y-3">
      {posts.map((post) => (
        <li
          key={post.id}
          className="rounded-md border border-border-default p-3 dark:border-border-default"
        >
          <p className="font-medium text-fg-default">{post.title || t('boards.moderation.untitled')}</p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="rounded-md border border-border-strong px-3 py-1.5 text-sm dark:border-border-default disabled:opacity-50"
            >
...
function ReportList({
  ...
  const { t } = useTranslation('common')
  if (reports.length === 0) {
    return <p className="text-sm text-fg-muted">{t('boards.moderation.emptyReports')}</p>
  }
  return (
    <ul className="space-y-3">
      {reports.map((report) => (
        ...
        >
          ...
          {report.reason ? (
            <p className="mt-1 text-sm text-fg-muted">{report.reason}</p>
          ) : null}
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              type="button"
