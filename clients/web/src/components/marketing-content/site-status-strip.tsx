    ...
    <InlineAlert tone={failed ? 'danger' : 'info'}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="flex items-center gap-2">
          {build.runUrl ? <a className="inline-flex items-center gap-1 text-accent-fg underline" href={build.runUrl} target="_blank" rel="noreferrer">View run <ExternalLink className="h-3.5 w-3.5" /></a> : null}
          {canRebuild ? <Button size="sm" variant="secondary" loading={rebuilding} onClick={onRebuild}><RefreshCw className="h-4 w-4" /> Rebuild site</Button> : null}
        </span>
      </div>
    </InlineAlert>
  )
