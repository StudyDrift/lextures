...
| **Markets** | K12 / HE |
| **Status (today)** | **DONE** — outcomes CRUD, item/question mapping with measurement/intensity levels, and read-only class progress on iOS and Android |
...
- **Logic**: `CourseOutcomesLogic` (iOS/Android) — gradable options, draft dirty detection, link payload builders, progress/link label helpers, cache/idempotency keys.
- **API**: `LMSAPIOutcomes.swift` + `LmsApi` methods for `GET/POST/PATCH/DELETE /outcomes` and `POST/DELETE …/links`.
- **Models**: `LMSFeatureModelsCourseOutcomes` / `LmsFeatureModelsCourseOutcomes.kt`.
- **UI**: `CourseOutcomesSettingsView` / `CourseOutcomesSettingsScreen` wired into M13.1 settings shell (`outcomes` section); outcome cards with edit, class progress, link mapping (assignment/quiz/question), offline queued writes; `UnsavedChangesBanner` for outcome edits.
- **Tests**: `CourseOutcomesLogicTests` (iOS), `CourseOutcomesLogicTest` (Android). iOS + Android unit tests pass locally.
