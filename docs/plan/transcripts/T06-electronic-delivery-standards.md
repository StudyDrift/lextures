...
> Implementation plan. The transport layer: PESC/EDI/SPEEDE + signed-PDF adapters, secure links, and delivery receipts. Replaces the fire-and-forget webhook. Source landscape: [transcripts/README](README.md).
...
| **Severity** | BLOCKER |
| **Markets** | HE · K12 |
| **Status (today)** | MISSING (as real delivery) — the only "delivery" is a single HMAC-signed JSON POST to one institution URL that carries *student info*, not a transcript. There is no standards-based electronic exchange, no per-recipient adapter, no secure download link, and no delivery/open receipt. |
...
Migration `383_transcript_delivery.sql` (indicative):
...
- Related plans: [T01](../../completed/transcripts/T01-official-transcript-generation.md), [T03](../../completed/transcripts/T03-order-lifecycle-fulfillment-holds.md), [T07](T07-inbound-receiving-transfer-credit.md), [T08](T08-credential-verification.md), [T10](T10-order-tracking-notifications.md).
