      ...
      {showTooltip && pos &&
        createPortal(
          <div
            ...
            className="tooltip-in lx-overlay-tooltip-in fixed z-[100] -translate-y-1/2 whitespace-nowrap rounded-lg bg-slate-950 px-2.5 py-1.5 text-xs font-semibold text-white shadow-xl ring-1 ring-white/10 dark:bg-surface-overlay"
          >
            {content}
            <div className="absolute -start-1 top-1/2 h-2 w-2 -translate-y-1/2 rotate-45 bg-slate-950 dark:bg-surface-overlay" />
          </div>,
          document.body,
        )}
    </div>
