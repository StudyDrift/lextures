...
> Implementation plan. Source: net-new capability (interactive game-based quizzing). Landscape: [interactive-quizzes/README](../../plan/interactive-quizzes/README.md). Consumes the timing + correctness recorded by [IQ.3](IQ.3-live-game-hosting-engine.md) and drives the feedback rendered by [IQ.4](IQ.4-player-join-and-gameplay.md).
...
Migration `399_interactive_quizzes_scoring.sql` (small — most state lives on IQ.3 tables; plan originally said `394`, already taken by board templates):
...
- **Sequencing:** migration `399` → scoring module + leaderboard → profile picker → enable.
...
- Related plans: [IQ.3](IQ.3-live-game-hosting-engine.md), [IQ.4](IQ.4-player-join-and-gameplay.md),
  [IQ.6](IQ.6-game-modes-team-paced-async.md), [IQ.7](../../plan/interactive-quizzes/IQ.7-reports-results-gradebook.md),
