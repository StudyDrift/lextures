...
> Implementation plan. Source: mobile parity for real-time boards over the **existing** relay. Landscape: [visual-collaboration/README](../../plan/visual-collaboration/README.md). Mirrors web [VC.4](VC.4-realtime-collaboration-and-presence.md) and connects to the **same** board WebSocket (`server/internal/httpserver/board_ws.go`), reusing the native `WebSocketClient` + per-screen socket pattern already shipped for feed/courses/notifications.
