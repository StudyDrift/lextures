...
| **CT.5** | [Tool SDK, sandboxing, versioning & migration](CT.5-tool-sdk-sandboxing-and-versioning.md) | MAJOR | M | CT.1, CT.3 | `@lextures/tool-sdk`, iframe sandbox + postMessage bridge, semver pinning, state migrations, bundle budgets |
| **CT.6** | [Grounded context service & web-link ingestion](CT.6-grounded-context-and-link-ingestion.md) | BLOCKER | M | CT.1 | Context packs from the activity, SSRF-safe link fetch/extract/cache, `fetch_link` tool-calling for AI tools |
| **CT.7** | [Analytics, insights & gradebook bridge](CT.7-analytics-insights-and-gradebook.md) | MAJOR | M | CT.3 | Per-tool instructor insights, struggle detection, xAPI/Caliper emission, optional score passback |
...
| **CT.14** | [Sort & Sequence](CT.14-tool-sort-and-sequence.md) — drag items into categories or into order | Schema building / procedural fluency | MAJOR | none |
