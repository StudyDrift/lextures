...
export default function PublicBadgeDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-base text-fg-default dark:bg-slate-950 dark:text-slate-100">
      <main className="mx-auto max-w-2xl px-4 py-10">
        <p className="mb-4 text-sm">
          <Link to={`/badges/${encodeURIComponent(handle)}`} className="text-accent-fg hover:underline dark:text-indigo-400">
            ← All badges
          </Link>
        </p>
        ...
        {badge ? (
          <article className="rounded-2xl border border-border-default bg-surface-raised p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-100 text-accent-fg dark:bg-indigo-950 dark:text-indigo-300">
            ...
            {badge.description ? (
              <p className="mt-4 text-sm text-fg-muted dark:text-slate-300">{badge.description}</p>
            ) : null}
            {badge.criteriaNarrative ? (
              <section className="mt-4">
                <h2 className="text-sm font-semibold">Criteria</h2>
                <p className="mt-1 text-sm text-fg-muted dark:text-slate-300">{badge.criteriaNarrative}</p>
              </section>
            ) : null}
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <button
                ...
                className="rounded-lg bg-accent-solid px-3 py-2 text-sm font-medium text-white hover:bg-indigo-500"
              ...
              {badge.verifyUrl ? (
                <a
                  ...
                  className="rounded-lg border border-border-strong px-3 py-2 text-sm font-medium hover:bg-surface-base dark:border-slate-600 dark:hover:bg-slate-800"
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            >
              ...
              {verifyState === 'revoked' ? (
                <>
                  ...
                  <span className="text-warning-fg dark:text-amber-400">{verifyMessage}</span>
                </>
              ) : null}
              {verifyState === 'unverified' || verifyState === 'error' ? (
                <>
                  <XCircle className="h-4 w-4 text-danger-fg" aria-hidden />
                  <span className="text-danger-fg dark:text-red-400">{verifyMessage}</span>
                </>
              ) : null}
              {verifyState === 'loading' ? (
                <>
