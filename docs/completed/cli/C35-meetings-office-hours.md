...
> CLI parity plan. Source: `registerMeetingRoutes` (`meetings`, `courses/{id}/meetings`), `registerOfficeHoursRoutes` + `office_hours_get.go` (`slots`, `courses/{id}/availability`), `conferences.go` (`conference-slots`), `calendar_http.go` + `registerCalendarFeedRoutes` (`me/calendar-token`). Baseline: `clients/cli/cmd/meetings_office_hours.go`, `meetings_office_hours_logic.go`, `meetings_office_hours_test.go`.
