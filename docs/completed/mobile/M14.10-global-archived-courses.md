...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — Archived courses admin on iOS and Android (list, search, restore, permanent delete) |
...
- **Logic & API**: `ArchivedCoursesAdminLogic` + `LMSFeatureModelsArchivedCoursesAdmin` / `LmsFeatureModelsArchivedCoursesAdmin.kt`; `LMSAPIAdmin.swift` / extended `LmsApi.kt` for `GET /api/v1/settings/archived-courses`, `POST …/restore`, `DELETE …`.
- **UI**: `ArchivedCoursesAdminView` / `ArchivedCoursesAdminScreen` — searchable list with title, course code, archived-by, archived-at; restore confirmation; permanent delete with type-course-code-to-confirm sheet/dialog and irreversible warning.
- **Profile entry**: `ArchivedCoursesAdminEntryCard` (iOS) and Profile tab nav row (Android), gated by `ffMobileAdminSettings` + `global:app:rbac:manage`.
- **Feature flag**: `ffMobileAdminSettings` on `PlatformFeatures` / `MobilePlatformFeatures` (default off).
- **Tests**: `ArchivedCoursesAdminLogicTests` (iOS), `ArchivedCoursesAdminLogicTest` (Android). iOS build + Android lint and unit tests pass.
...
1. Should permanent delete be allowed on mobile at all, or restore-only + delete on web?
   — **Resolved:** Both restore and permanent delete ship on mobile; delete requires typing the course code.
2. Exact archived-courses endpoint shapes.
   — **Resolved:** `GET /api/v1/settings/archived-courses` → `{ courses: ArchivedCourseRow[] }`; restore `POST …/{course_code}/restore`; delete `DELETE …/{course_code}` (204, server-audited).
