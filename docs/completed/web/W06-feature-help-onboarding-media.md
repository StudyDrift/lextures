...
- **Placeholder removed:** `feature-help-dock.tsx` no longer ships "placeholder for a demo GIF" copy; the media region is omitted when a feature has no clip.
- **Media registry:** `FEATURE_HELP_MEDIA` in `clients/web/src/lib/feature-help-content.ts` maps optional per-topic `src` + i18n `altKey`; first bundled clip is `modules` (`/feature-help/modules-walkthrough.mp4`).
- **Component:** `FeatureHelpMediaRegion` lazy-loads on panel open (`preload="none"`), shows a skeleton while loading, and degrades to text-only help on error.
- **i18n:** `featureHelp.media.*` alt keys in `common` namespace (`en`/`es`/`fr`/`ar`).
- **Tests:** Vitest (`feature-help-content.test.ts`, `feature-help-dock.test.tsx`); Playwright (`e2e/tests/feature-help.spec.ts`).
