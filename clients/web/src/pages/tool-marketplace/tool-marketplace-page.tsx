  ...
  if (!ffContentToolMarketplace && !featuresLoading) {
    return (
      <div className="mx-auto max-w-3xl p-6" data-testid="tool-marketplace-disabled">
        <p className="text-sm text-slate-600">{t('contentTools.marketplace.disabled')}</p>
      </div>
    )
  }
    ...
    <div className="mx-auto max-w-4xl p-6" data-testid="tool-marketplace-page">
      ...
      <label className="mt-4 block text-sm">
        ...
        <input
          ...
          className="mt-1 w-full rounded border border-slate-300 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-900"
          value={q}
      ...
      <ul className="mt-6 space-y-4" aria-labelledby={titleId}>
        {tools.map((tool) => (
          <li key={tool.toolId} className="border-b border-slate-200 pb-4 dark:border-slate-700">
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{tool.summary}</p>
            <p className="mt-2 text-xs text-slate-500">{tool.toolId}</p>
            <Link
      ...
      {!loading && tools.length === 0 ? (
        <p className="mt-4 text-sm text-slate-600">{t('contentTools.marketplace.empty')}</p>
      ) : null}
    </div>
  )
}
