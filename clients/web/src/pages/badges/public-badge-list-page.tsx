    ...
    <div className="min-h-screen bg-surface-base text-fg-default dark:bg-slate-950 dark:text-slate-100">
      <main className="mx-auto max-w-3xl px-4 py-10">
        <header className="mb-8">
          ...
          {resolvedHandle ? (
            <p className="mt-1 text-sm text-fg-muted">@{resolvedHandle}</p>
          ) : null}
        </header>
        ...
        {!loading && status === 'private' ? (
          <p className="rounded-xl border border-border-default bg-surface-raised p-6 text-sm dark:border-slate-700 dark:bg-slate-900">
            This badge page is private.
          </p>
        ) : null}

        {!loading && status === 'ok' && badges.length === 0 ? (
          <p className="text-sm text-fg-muted dark:text-slate-300">No public badges yet.</p>
        ) : null}
          ...
          {badges.map((b) => (
            <li key={b.id}>
              <Link
                ...
                className="block rounded-xl border border-border-default bg-surface-raised p-4 shadow-sm transition-[background-color,color,border-color] hover:border-indigo-300 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-indigo-600"
              >
                ...
                {b.description ? (
                  <p className="mt-2 line-clamp-3 text-sm text-fg-muted dark:text-slate-300">{b.description}</p>
                ) : null}
