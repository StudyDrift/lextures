...
| **Markets** | K12 / HE |
| **Status (today)** | **DONE** — sections list/create/rename/archive, roster assignment, due-date overrides, and cross-listing on iOS and Android |
...
- **Logic**: `CourseSectionsLogic` (iOS/Android) — sections gating, active-section filter, roster counts, assignment picker helpers, override payload builder, cross-list permission (`tenant:org:units:admin`), offline idempotency keys, cache key.
- **API**: `LMSAPICourseSections.swift` + `LmsApi` methods for section CRUD, enrollment section transfer, section assignment overrides, and org cross-list groups/members.
- **Models**: Extended `CourseSection` with `status`; added cross-list and override request/response types.
- **UI**: `CourseSectionsSettingsView` / `CourseSectionsSettingsScreen` wired into M13.1 settings shell (`sections` section); `CourseCrossListingView` / `CourseCrossListingSection` for org-admin cross-list management with confirmations; section detail sheet with rename, archive, roster move; offline queued writes.
- **Tests**: `CourseSectionsLogicTests` (iOS), `CourseSectionsLogicTest` (Android). iOS build + unit tests pass locally. Web e2e `sections-settings.spec.ts` covers API persistence.
