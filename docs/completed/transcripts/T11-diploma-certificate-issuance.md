...
> Implementation plan. Institution-issued, verifiable diplomas and certificates that land in the learner wallet. Source landscape: [transcripts/README](../../plan/transcripts/README.md).
...
| **Severity** | MINOR |
| **Markets** | HE · K12 |
| **Status (today)** | DONE — diploma/certificate templates, single + batch issuance, signed PDF + VC, T08 verify, T09 wallet surfacing, revoke/unrevoke; migration `414`, flag `ff_diplomas` (default off). Learner list/download at `/api/v1/me/diplomas` (avoids collision with completion-credentials `/me/credentials`). |
...
Migration `414_diplomas_certificates.sql` (implemented). Tables:
`credentials.diploma_templates` (not `credential_templates` — that name is taken by plan 15.5),
`credentials.diplomas`, `credentials.diploma_batches`, `credentials.diploma_batch_items`,
plus `ff_diplomas` on `settings.platform_app_settings`.
...
- `POST /api/v1/admin/credentials/issue/batch` — cohort/program batch (async job `diploma.issue.batch`).
- `GET  /api/v1/admin/credentials/batches/{id}` — batch progress.
- `POST /api/v1/admin/credentials/{id}/revoke|unrevoke`.
- `GET  /api/v1/me/diplomas` — my diplomas/certificates (also surfaced via wallet T09).
- `GET  /api/v1/me/diplomas/{id}/download` — signed PDF.
- Verification via T08 (`/verify/{token}`). OpenAPI updated; `FFDiplomas` / `ff_diplomas` in `platformconfig`.
