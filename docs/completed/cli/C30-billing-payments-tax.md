...
> CLI parity plan. Source: `billing_http.go` (`billing`), `payments_http.go` (`payments`, `checkout`, `invoices`), `tax_http.go` (`orgs/{orgId}/tax`), `registerRevenueShareRoutes` (`revenue`), `affiliate`, `me/transactions`. Baseline: `clients/cli/cmd/billing.go`, `invoices.go`, `payments.go`, `tax.go`, `billing_payments_logic.go`, `billing_payments_test.go` (extends `revenue.go`).
