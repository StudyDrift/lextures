...
| **Status (today)** | DONE — module list with lock/completion state, native content page reader, item routing, progress sync, and offline cache on iOS and Android |
| **Estimated effort** | M |
...
- H5P/SCORM/LTI runtime is [M3.3](../completed/mobile/M3.3-interactive-content.md) (here we route to it
...
- **FR-4.** Item taps MUST route by type: page → reader; quiz → [M4.1](M4.1-quiz-taker.md);
  ...
  resource → reader/web view; H5P/SCORM/LTI → [M3.3](../completed/mobile/M3.3-interactive-content.md)
...
3. ✅ A content page renders text, images, embedded video URLs, and LaTeX-style math
   and scrolls smoothly.
4. ✅ Each item type routes to the right destination (quiz/assignment/file/interactive
   placeholders; external/textbook → in-app web view).
5. ✅ Completing an item updates progress via `POST …/items/{id}/complete` and refreshes
   lock state from `GET …/modules/progress`.
6. ✅ Offline, cached structure/content pages render with staleness chip; completion
   writes queue via the outbox.
