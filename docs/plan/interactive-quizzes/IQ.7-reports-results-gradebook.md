...
> Implementation plan. Source: net-new capability (interactive game-based quizzing). Landscape: [interactive-quizzes/README](README.md). Reuses item analysis (`itemanalysis`), grade sync (`quiz_grade_sync.go`), and learner-progress plumbing already in the platform.
...
Migration `396_interactive_quizzes_reports.sql`:
...
- **Flag:** `interactive_quizzes_enabled`; gradebook push behind a sub-flag until verified against known
  scores.
- **Sequencing:** migration `396` → report build + pages → export → gradebook link.
...
- Related plans: [IQ.3](../../completed/interactive-quizzes/IQ.3-live-game-hosting-engine.md), [IQ.5](IQ.5-scoring-leaderboards-mechanics.md),
  [IQ.6](IQ.6-game-modes-team-paced-async.md), [IQ.11](IQ.11-admin-governance-quotas-lifecycle.md).
