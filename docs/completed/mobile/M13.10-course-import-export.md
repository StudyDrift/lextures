...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — JSON export (share sheet) + import/restore (document picker, confirm, progress) on iOS and Android; QTI/CC/Canvas link-out card |
| **Estimated effort** | S (1w) |
| **Platforms** | iOS (SwiftUI), Android (Compose) |
| **Backend** | None — `GET /api/v1/courses/{code}/export` and `POST /api/v1/courses/{code}/import` (parity with web) |
...
- **Logic**: `CourseImportExportLogic` (iOS/Android) — import modes (`erase` / `mergeAdd` / `overwrite`), JSON validation, 50 MB on-device cap, web path helper, error mapping.
- **API**: `LMSAPICourseImportExport.swift` + `LmsApi.fetchCourseExport` / `postCourseImport` calling `GET/POST /api/v1/courses/{code}/export|import`.
- **UI**: `CourseImportExportView` / `CourseImportExportScreen` wired into the M13.1 settings shell (`importExport` section); export → temp file → share sheet; import → document picker → confirm dialog → progress/success/error; QTI/CC/Canvas card links to web `/courses/{code}/settings/import-export`.
- **Tests**: `CourseImportExportLogicTests` (iOS), `CourseImportExportLogicTest` (Android). iOS + Android builds pass; unit tests pass.
...
- Export via `GET /api/v1/courses/{code}/export` (JSON backup) and restore via
  `POST /api/v1/courses/{code}/import` with `{ mode, export }` (matches web
  `course-export-import-section.tsx`).
...
- **iOS:** `Features/Courses/Settings/CourseImportExportView.swift`; API in
  `Core/LMS/LMSAPICourseImportExport.swift`; `fileImporter` + share sheet.
- **Android:** `features/courses/settings/CourseImportExportScreen.kt`; Storage Access Framework
  (`OpenDocument` / `ACTION_SEND`); `core/lms/LmsApi.kt`.
...
1. ~~Exact export report type / import endpoint shapes to confirm.~~ Resolved: web uses `GET …/export` and `POST …/import`.
2. ~~Max file size we support on-device before requiring web.~~ 50 MB cap in `CourseImportExportLogic`.
