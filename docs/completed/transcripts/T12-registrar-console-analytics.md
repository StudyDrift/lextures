...
> Implementation plan. The registrar's operational home + destination/volume/revenue/SLA analytics. Source landscape: [transcripts/README](../../plan/transcripts/README.md).
...
| **Severity** | MAJOR |
| **Markets** | HE · K12 |
| **Status (today)** | DONE — registrar console tab shell (queue/holds/fees/delivery/recipients/settings/analytics/inbound); dashboard + SLA health + CSV export APIs; daily rollups (`transcripts.analytics_daily`) + turnaround view; panel RBAC permissions; migration `416`, scheduled `transcript_analytics_rollup`. |
...
- Implemented: `repos/transcripts/analytics.go`, `httpserver/transcripts_analytics_http.go`, `background/transcript_analytics.go`, migration `416_transcript_analytics.sql`, admin UI `AdminTranscripts.tsx` + `transcript-analytics-panel.tsx`, client `transcripts-api.ts` dashboard/health/export helpers.
- Existing: admin config in `transcripts_http.go`, platform settings (`transcripts-settings-panel.tsx`), observability ([17.7](../../plan/17-platform-performance-operability/)), [09 analytics](../09-analytics-reporting/).
- Related plans: [T02](T02-recipient-directory-and-orders.md), [T03](T03-order-lifecycle-fulfillment-holds.md), [T05](T05-fees-payments-waivers.md), [T06](T06-electronic-delivery-standards.md).
