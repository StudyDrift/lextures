...
const publicColumns = `a.id,a.kind,a.slug,a.locale,a.translation_group_id,a.category_id,a.path,
 ...
 a.canonical_override,a.extra,a.revision_no,a.created_by,a.updated_by,a.created_at,a.updated_at,a.deleted_at,c.slug,c.title,
...
func ListPublishedArticles(ctx context.Context, q querier, f PublicArticleFilter) ([]PublicArticle, string, error) {
 ...
 AND ($6='' OR a.search_tsv @@ websearch_to_tsquery('english',$6)) AND ($7::timestamptz IS NULL OR (a.published_at,a.id)<($7,$8))
...
func SearchPublished(ctx context.Context, q querier, query, kind string, limit int) ([]SearchResult, error) {
	if limit < 1 {
		limit = 10
	}
	if limit > 50 {
	...
	rows, e := q.Query(ctx, `WITH x AS (SELECT websearch_to_tsquery('english',$1) q) SELECT a.path,a.title,a.description,ts_headline('english',a.body_md,x.q,'MaxWords=30,MinWords=10,StartSel=<mark>,StopSel=</mark>'),a.kind,ts_rank_cd(a.search_tsv,x.q) FROM marketing.content_articles a,x WHERE $1<>'' AND a.status='published' AND a.deleted_at IS NULL AND a.published_at<=now() AND a.search_tsv@@x.q AND ($2='' OR a.kind=$2) ORDER BY 6 DESC,a.published_at DESC LIMIT $3`, query, kind, limit)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
	...
	for rows.Next() {
		var s SearchResult
		if e = rows.Scan(&s.Path, &s.Title, &s.Description, &s.Snippet, &s.Kind, &s.Rank); e != nil {
			return nil, e
		}
		out = append(out, s)
	}
