...
- [TD — Technical Debt Remediation](tech_debt/) — structural remediation of the four god-structures that now gate velocity: the 390-file / 125K-LOC flat `internal/httpserver` package, the 32-field `Deps` and 335-field `Config`, the 8,631-line `courses-api.ts`, and the 3,300-line / 99-`useState` course screens — plus 223 proven-dead Go functions, 130 raw DB calls in the HTTP layer, and a **confirmed live defect: `/api/openapi.json` serves invalid JSON**. Guardrails first (TD.1–TD.2), then defects and dead code (TD.3–TD.5), backend architecture (TD.6–TD.10), frontend architecture (TD.11–TD.14). Prime directive: **no user-visible behaviour may change** (TD.1–TD.14)
