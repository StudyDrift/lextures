...
> `components/settings/*-panel.tsx`. Follows [the mobile plan template](../../plan/_TEMPLATE.md), mobile-tuned.
...
| **Markets** | HE / K12 |
| **Status (today)** | **DONE** — iOS and Android provide permission-gated status views and enable/disable toggles for LTI, SCIM, cloud, LRS, and OER |
...
- **Access and rollout:** Profile entry and screens require `ffMobileAdminSettings` plus
  `global:app:rbac:manage`. Unauthorized views show the permission fallback and do not call
  admin integration endpoints.
- **Hub:** `IntegrationsAdminView` / `IntegrationsAdminScreen` lists available sections. LTI and
  cloud always appear for admins; SCIM requires platform `scimEnabled` (from
  `GET /api/v1/settings/platform`); LRS requires `xapiEmissionEnabled`; OER requires
  `oerLibraryEnabled` (from platform features).
- **LTI:** `GET /api/v1/admin/lti/registrations`; enable/disable via
  `PUT .../registrations/{id}` and `PUT .../external-tools/{id}` with `{active}` and confirmation.
- **SCIM:** status-only — active token count and recent events for an institution (admin org
  picker). Bearer token create/revoke stays on web; secrets never shown.
- **Cloud / LRS / OER:** list status rows and toggle `enabled` with confirmation. Mobile models
  omit cloud credentials (`clientId`/`apiKey`/`appKey`); toggles send `{enabled}` only. LRS
  passwords/oauth secrets are not decoded.
- **Link-out:** each detail screen has “Configure on web” to the matching settings path.
- **Logic & API:** `IntegrationsAdminLogic` + `LMSFeatureModelsIntegrationsAdmin` /
  `LmsFeatureModelsIntegrationsAdmin.kt`; methods on `LMSAPIAdmin.swift` / `LmsApi.kt`.
  `xapiEmissionEnabled` added to platform feature models.
- **Tests:** `IntegrationsAdminLogicTests` (iOS) and `IntegrationsAdminLogicTest` (Android) cover
  gating, section visibility, secret-field exclusion, decode, and toggle helpers. Both suites pass.
- **i18n:** 55 keys under `mobile.admin.integrations.*` in `clients/mobile/locales`, synced to iOS
  catalog and Android `strings.xml`.
