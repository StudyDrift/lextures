...
> **Accessibility** tab, flag `altTextEnforcementEnabled`). Follows [../_TEMPLATE.md](../_TEMPLATE.md),
> mobile-tuned.
...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — alt-text coverage review and inline fixes on iOS and Android; gated by `altTextEnforcementEnabled` |
...
- **Logic**: `CourseAccessibilityReviewLogic` (iOS/Android) — markdown image scan, coverage percent, alt-text apply/replace, pagination, inline-edit gating (`content_page` + `assignment`), cache/idempotency keys.
- **API**: `LMSAPICourseAccessibility.swift` + `LmsApi` methods for `GET /accessibility`, `POST /alt-text/suggest`, and `PATCH` content-pages/assignments markdown.
- **Models**: `LMSFeatureModelsCourseAccessibility` / `LmsFeatureModelsCourseAccessibility.kt`.
- **UI**: `CourseAccessibilityReviewView` / `CourseAccessibilityReviewScreen` wired into M13.1 settings shell (`accessibility` section); coverage ring/bar + gap list with pagination; item fixer sheet (image preview, alt field, decorative toggle, optional AI suggest); offline queued markdown writes; refetch coverage after save.
- **Scope**: Inline editing for content pages and assignments; other kinds show link-out to web.
- **Tests**: `CourseAccessibilityReviewLogicTests` (iOS), `CourseAccessibilityReviewLogicTest` (Android). iOS + Android unit tests pass locally.
...
- **iOS:** `Features/Courses/Settings/CourseAccessibilityReviewView.swift`; API in
  `Core/LMS/LMSAPICourseAccessibility.swift`.
...
1. Which content types support inline alt-text editing on mobile vs. link-out? **Resolved:** `content_page` and `assignment` inline; others link-out.
2. Exact coverage endpoint shape. **Resolved:** matches web `CourseAccessibilityInfo` (`altTextCoverage`, `hardBlockSave`).
