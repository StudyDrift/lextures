...
| **Markets** | K12 / HE / SL |
| **Status (today)** | **DONE** — Org branding (logo upload, colors, email sender, custom-domain read + web link-out), AI governance toggles/limits, and AI provider settings on iOS and Android |
...
- **Logic & API**: `OrgBrandingAdminLogic` + `LMSFeatureModelsOrgBrandingAdmin` / `LmsFeatureModelsOrgBrandingAdmin.kt`; extended `LMSAPIAdmin.swift` / `LmsApi.kt` for org branding GET/PUT, logo upload, AI config GET/PUT, AI provider settings GET/PUT, and connection test.
- **UI**: `OrgBrandingAdminView` / `OrgBrandingAdminScreen` host three cards — `OrgBrandingView` / `OrgBrandingScreen` (logo, colors, sender, contrast preview, custom domain read-only + web link-out), `AiGovernanceView` / `AiGovernanceScreen` (feature toggles + allowed models), `AiProviderSettingsView` / `AiProviderSettingsScreen` (provider/model/fallback + masked BYOK secret, test connection).
...
- **Security**: BYOK uses web's `••••••••••••` placeholder pattern; secrets write-only, never logged or re-displayed after save.
- **Tests**: `OrgBrandingAdminLogicTests` (iOS), `OrgBrandingAdminLogicTest` (Android). iOS build + unit tests pass.
- **Out of scope on mobile**: custom-domain DNS/verification (web link-out only).
...
1. ~~Do we allow logo upload on mobile, or edit colors/sender only?~~ **Resolved:** logo upload supported (same endpoint as web).
2. ~~Exact governance fields + provider secret semantics to mirror.~~ **Resolved:** mirrors web panels; BYOK placeholder `••••••••••••`.
