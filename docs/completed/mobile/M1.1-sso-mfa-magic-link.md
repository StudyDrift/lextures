...
| **Status (today)** | DONE — SSO (ASWebAuthenticationSession / Custom Tabs), TOTP/backup MFA, magic-link deep links, SAML `forceSaml` routing; iOS passkeys; Android passkey UI stub pending Credential Manager |
| **Estimated effort** | M |
| **Platforms** | iOS (SwiftUI), Android (Compose) |
| **Backend** | No changes — `saml-callback`, `oidc`, `mfa-login`, `magic-link` flows exist server-side |
| **Depends on** | Auth session (built) · pairs with [M1.2](../../plan/mobile/M1.2-biometric-sessions.md) |
...
- Building MFA enrollment management UI beyond completing a challenge + minimal
  enrollment (full device management is [M1.2](../../plan/mobile/M1.2-biometric-sessions.md)/web).
...
- **FR-5.** All resulting tokens MUST land in the existing **secure store**; refresh
  and revocation behave as in [M1.2](../../plan/mobile/M1.2-biometric-sessions.md).
...
- **Web bridge**: `clients/web/src/pages/saml-callback.tsx` redirects to
  `lextures://auth/callback` when `next=/__mobile_callback__`.

## 9. Files touched

**iOS** — `Features/Auth/SSOAuthController.swift`, `Features/Auth/MFAChallengeView.swift`,
`Features/Auth/MfaPasskeyController.swift`, `Features/Auth/LoginView.swift`,
`Features/Auth/AuthFlowView.swift`, `Core/Auth/AuthAPI.swift`, `Core/Auth/AuthCallbackParser.swift`,
`Core/Auth/AuthConstants.swift`, `Core/Auth/AuthSession.swift`, `Resources/Info.plist`,
`LexturesTests/AuthCallbackParserTests.swift`, `project.pbxproj`.

**Android** — `features/auth/SsoAuth.kt`, `features/auth/MfaChallengeScreen.kt`,
`features/auth/LoginScreen.kt`, `features/auth/AuthFlowScreen.kt`, `core/auth/AuthApi.kt`,
`core/auth/AuthCallbackParser.kt`, `core/auth/AuthConstants.kt`, `core/auth/AuthSession.kt`,
`app/src/main/AndroidManifest.xml`, `AuthCallbackParserTest.kt`.

**Web** — `clients/web/src/pages/saml-callback.tsx` (mobile token redirect).
...
- SSO callback uses `next=/__mobile_callback__`; web `saml-callback` forwards tokens to
  `lextures://auth/callback?…`.
- Magic-link App Links: `/login/magic-link` on iOS (AASA) and Android.
- Android MFA passkey: TOTP and backup codes ship; native passkey via Credential Manager
  is a follow-up (UI shows unsupported message today).
