...
| **Status (today)** | DONE — biometric gate + 401 refresh/retry + session list/revoke on iOS and Android |
| **Estimated effort** | S |
...
- **iOS**: `BiometricGate`, `NetworkAuthContext`, `SessionsAPI`, `DeviceSessionsView`, `BiometricLockView`; unit tests in `BiometricGateTests`.
- **Android**: `BiometricGate`, `NetworkAuthContext`, `SessionsApi`, `DeviceSessionsScreen`, `BiometricLockScreen`; unit tests in `BiometricGateTest`.
- **Locales**: `mobile.biometric.*`, `mobile.sessions.*`, `mobile.profile.security`, `mobile.auth.sessionRevoked` in all mobile locale files.
- **Platform flag**: device session API requires `SessionManagementUIEnabled` (same as web account settings).
